...
## TLDR
This plotter represents functions $f: \mathbb{C} \rarrow \mathbb{C}$. The input's real and imaginary components are represented as spacial axes, and the ouput is mapped to a color. This is what you see in your screen represents. 
...
In mathematics, we can shorthand the sentence "the function $f$ takes input $A$ and outputs $B$" as simply $f:A \rarrow B$. We will be using this notation moving forward.

Usually, we work with real functions, this just being a function $f:\mathbb{R} \rarrow \mathbb{R}$. It's likely you've come across one of such functions like $f(x) = x^2$. We can observe how this function behaves by tabulating the values of $x$ and $f(x)$

{MANIM PLOT SHOWING A TABLE OF X vs F(X)}
...
{MANIM PLOT OF TWO PARALELL AXES X AND F(X) }
...
{MORE INTERMEDIATE POINTS BEING ADDED, UNTIL TRANSFORMING INTO A PARABOLA}

And voilà! This is the plot of the _parabola_, the visual representation of this function $f(x) = x^2$ we all know and love. 

## Dimensionality

This strategy for plotting functions works fine for functions $f:\mathbb{R} \rarrow \mathbb{R}$ because it takes one number for input, and produces one for output. Now, suppose we have a function with two inputs (which we would notate by $g:\mathbb{R} \times \mathbb{R} \rarrow \mathbb{R}$), like such:
...
This method is, however, limited. The set of the complex numbers ($\mathbb{C}$) has a dimensionality of 2. Suppose we want to plot a function $f:\mathbb{C} \rarrow \mathbb{C}$. With our current method, we can't, because $dim(f) = 4$, and we only have three spatial dimensions! 

{PLOT WHERE IT ADDS ONE AXIS, TWO AXIS, THREE AXIS [Re(z), Im(z), Re(f(z))] and then the third axis, Im(f(z)), moves arounds aimlessly trying to find somewhere to fit, but there isn't any.}
...
{Animation showing z = a + bi, highlighting a,b and i in different colors, and pulling arrows saying "real part", "imaginary part" and "imaginary unit"}
...
{Animation showing the process of obtaining the expressions of (a+bi)*(c+di) and its inverse}
...
{Simple plotting of some points in this way}
...
{Tell me if this is something you can generate manually, or if you need an image: the HSL color wheel, and then a bar coming from the center with a length of l (cursive) and angle of theta. These two things are overlaid over the image of the HSL color wheel. Then, outside there is a little "legend" showing the values of l,theta and the current color shown in hex}

Since the dimensionality of color and a complex number are the same, we can map one to another! There are many methods to do so, the one used in this plotter being disclosed [here](), but this means we can now plot functions $f: \mathbb{C} \rarrow \mathbb{C}$! In simple terms: the axes serve as input, and the colors, as output. 
...
{Animation of f(z) = z^2 warping the standard grid}
...
{Plot of f(z) = z^2, which i will make myself}

All checks out! The grid is being warped, and the colors are being repeated (notice how red shows up twice). There are many fun transformations to explore, and I incentivize you to try them out yourself! 
